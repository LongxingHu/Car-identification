...
def train_model(network, criterion, optimizer, trainLoader, valLoader, n_epochs = 200):
    ...
    for epoch in range(0, n_epochs):
        ...
        #     torch.save(network, 'fine_tune/network{}.pt'.format(epoch+1))


def train_model_(network, criterion, optimizer, trainLoader, valLoader, n_epochs = 200):
    network = network.cuda()
    criterion = criterion.cuda()
...
def test_model(network, criterion, valLoader):
    network = network.cuda()
    criterion = criterion.cuda()
    correct = 0.0
    cum_loss = 0.0
    ...
    print("Top1 Accuracy")
    t = tqdm(valLoader, desc = 'Validation epoch %d' % 1)
    network.eval()  # This is important to call before evaluating!
    with torch.no_grad():
        for (i, (inputs, labels)) in enumerate(t):
            ...
            # logging information.
            cum_loss += loss.data.item()
            max_scores, max_labels = outputs.data.topk(1, dim=1)
            #for j in range(5):
            #    correct += (max_labels[:,j] == labels.data).sum()
            correct += (max_labels[:,0] == labels.data).sum()
            counter += inputs.size(0)
            t.set_postfix(loss = cum_loss / (1 + i), accuracy = 100 * correct / counter)
...
def test_model_top5(network, criterion, valLoader):
    network = network.cuda()
    criterion = criterion.cuda()
    correct = 0.0
    cum_loss = 0.0
...
def main():
    ...
    model = torch.load('fine_tune/network_all_100.pt')
    model.cuda()
    ...
    test_model_top5(model, criterion, testLoader)
